...
{cmd:help sfpanel}{right: ({browse "http://www.stata-journal.com/article.html?article=st0315":SJ13-4: st0315})}
{hline}
...
{pmore}i) the Battese and Coelli (1988) model, in which u_i is a
truncated normal distribution with nonzero mean and constant variance;
and

{pmore}ii) the Pitt and Lee (1981) model, in which u_i is a half-normal
distribution with constant variance.

{pstd}When estimation is done with least-squares methods, the SF
production model is

		y_it = alpha + beta*X_it + v_it

{pstd}Among the time-varying inefficiency models (alpha=alpha_it),
{cmd:sfpanel} fits

{pmore}i) the Lee and Schmidt (1993) model, in which alpha_it =
...
{pmore}ii) the Cornwell, Schmidt, and Sickles (1990) model, in which
alpha_it = delta_i0 + delta_i1*t + delta_i2*t^2.

{pstd}Among the time-invariant inefficiency models (alpha=alpha_i),
{cmd:sfpanel} fits the Schmidt and Sickles (1984) model, in which
alpha_i can be either fixed or random.
...
{phang}{cmd:emean(}{it:{help varlist:varlist_m}}[{cmd:,}
...
{phang}{cmd:usigma(}{it:{help varlist:varlist_u}}[{cmd:,}
...
{phang}{cmd:vsigma(}{it:{help varlist:varlist_v}}[{cmd:,}
...
{phang}{opt svfrontier(listname)} specifies a 1 x k vector of starting
values for the frontier.  The vector must have the same length as there
are parameters to estimate in the frontier equation.

{phang}{opt svemean(listname)} specifies a 1 x k_m vector of starting
values for variables parameterizing the truncation mean of the
inefficiency term.  It cannot be used when the distribution of the
inefficiency term is exponential or half-normal.

{phang}{opt svusigma(listname)} specifies a 1 x k_u vector of starting
values for variables parameterizing the variance of the inefficiency
term.

{phang}{opt svvsigma(listname)} specifies a 1 x k_v vector of starting
...
{phang}{opt rescale} determines whether rescaling is attempted.
...
{pstd}By default, the starting values come from an equivalent
cross-sectional frontier model.  For instance, if inefficiency has a
truncated normal distribution, and the random shock is a function of
some variables, {cmd:sfpanel} will first run {helpb sfcross} with a
truncated normal distribution and the 
{cmd:vsigma(}{it:{help varlist:varlist_v}}{cmd:)} option.  The estimated
coefficients are used as starting values.

{dlgtab:Other}

{phang}{opt cost} specifies that {cmd:sfpanel} fit a cost frontier
model.

{phang}{cmd:constraints(}{it:{help estimation options##constraints():constraints}}{cmd:)} applies specified linear constraints.
...
{phang}{cmd:robust} is the synonym for {cmd:vce(robust)}.

{phang}{opt cluster(clustvar)} is the synonym for {cmd:vce(cluster}
{it:clustvar}{cmd:)}.

{dlgtab:Reporting}

{phang}{opt feshow} allows the user to display estimates of individual
fixed effects, along with structural parameters.

{phang}{opt level(#)}; see {helpb estimation options##level():[R] estimation options}.

{phang}{opt nocnsreport}; see {helpb estimation options##nocnsreport:[R] estimation options}.
...
{phang}{opt cost} specifies that {cmd:sfpanel} fit a cost frontier model.
...
{phang}{opt nsimulations(#)} specifies the number of draws used in the
...
{phang}{opt cost} specifies that {opt sfpanel} fits a cost frontier
model.
...
{phang}{it:display_options}:  {opt noomit:ted}, {opt vsquish},
{cmdab:noempty:cells}, {opt base:levels}, and {opt allbase:levels}; see
{helpb estimation options##display_options:[R] estimation options}.
...
{phang}{opt cost} specifies that {opt sfpanel} fit a cost frontier
model.
...
{phang}{opt svemean(#)} specifies a starting value for the truncation
mean of the inefficiency term.

{phang}{opt svsigma(#)} specifies a starting value for the sigma
parameter.

{phang}{opt svgamma(#)} specifies a starting value for the gamma
parameter.

{phang}{opt sveta(#)} specifies a starting value for the variable
controlling the behavior of the firm effects over time.
...
{phang}{opt cost} specifies the frontier model be fit in terms of a cost
...
{phang}{cmd:bt(}{it:{help varlist:varlist_bt}}[{cmd:,}
...
{phang}{opt svbt(listname)} specifies a 1 x k_bt vector of initial
values for the coefficients of the time function controlling the
behavior of the firm effects.

{phang}{opt svusigma(#)} specifies a starting value for the parameter of
the variance of the inefficiency term.

{phang}{opt svvsigma(#)} specifies a starting value for the parameter of
...
{phang}{opt svusigma(listname)} specifies a starting value for the
parameter of the variance of the inefficiency term.

{phang}{opt svvsigma(listname)} specifies a starting value for the
...
{pstd}{cmd:sfpanel} stores the following in {cmd:e()}:
