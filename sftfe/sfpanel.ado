...
if "`model'"=="tfe" {
	...
	if "`distribution'"=="hnormal" {
		...
		if "`svfrontier'"=="" {
			...
			cap qui sfcross `lhs' `rhs', d(hn) difficult `cost' u(`u_rhs' `u_nocons') v(`v_rhs' `v_nocons') iter(50)
	...
	if "`distribution'"=="exponential" {
		...
		if "`svfrontier'"=="" {
			tempname init_params
			cap qui sfcross `lhs' `rhs', d(e) `cost' u(`u_rhs' `u_nocons') v(`v_rhs' `v_nocons') iter(50)
			local _sfcross_conv "`e(converged)'"
			if _rc != 0 qui reg `lhs' `rhs'		
			mat `init_params'=e(b)
			scalar `alpha' = `init_params'[1,`nsv_frontier']
	...
	if "`distribution'"=="tnormal" {
		...
		if "`svfrontier'"=="" {
			tempname init_params
			cap qui sfcross `lhs' `rhs', d(tn) `cost' em(`emean' `e_nocons') u(`usigma' `u_nocons') v(`vsigma' `v_nocons') difficult iter(50)
			local _sfcross_conv "`e(converged)'"
			if _rc != 0 qui reg `lhs' `rhs'
			mat `init_params' =e(b)
			scalar `alpha' = `init_params'[1,`nsv_frontier']
...
if "`model'"=="tre" {
	...
	if "`distribution'"=="hnormal" {
		...
		if "`svfrontier'"=="" {
			tempname init_params
			cap qui sfcross `lhs' `rhs', d(hn) `nocons' `cost' u(`u_rhs' `u_nocons') v(`v_rhs' `v_nocons') iter(30)
			local _sfcross_conv "`e(converged)'"
			if _rc != 0 qui reg `lhs' `rhs'
			mat `init_params'=e(b)
			mat `init_beta' = `init_params'[1,1..`nsv_frontier']
	...
	if "`distribution'"=="tnormal" {
		...
		if "`svfrontier'"=="" {
			tempname init_params
			cap qui sfcross `lhs' `rhs', d(tn) `nocons' `cost' em(`e_rhs' `e_nocons') u(`usigma' `u_nocons') v(`vsigma' `v_nocons') difficult iter(30)
			local _sfcross_conv "`e(converged)'"
			if _rc != 0 qui reg `lhs' `rhs'
			mat `init_params'=e(b)
			mat `init_beta' = `init_params'[1,1..`nsv_frontier']
	...
	if "`distribution'"=="exponential" {
		...
		if "`svfrontier'"=="" {
			tempname init_params			
			cap qui sfcross `lhs' `rhs', d(e) `cost' `nocons' u(`u_rhs' `u_nocons') v(`v_rhs' `v_nocons') iter(50)
			local _sfcross_conv "`e(converged)'"
			if _rc != 0 qui reg `lhs' `rhs'
			mat `init_params'=e(b)
			mat `init_beta' = `init_params'[1,1..`nsv_frontier']
...
tempname init_beta init_emean init_usigma init_vsigma
	...
	if "`svfrontier'"=="" {
		tempname init_params	
		cap qui sfcross `lhs' `rhs', d(tn) `nocons' `cost' em(`emean' `e_nocons') u(`usigma' `u_nocons') v(`vsigma' `v_nocons') difficult iter(50)
		local _sfcross_conv "`e(converged)'"
...
if "`model'"=="kumb90" {
	tempname init_beta init_bt init_usigma init_vsigma temp
		...
		if "`svfrontier'"=="" {
			tempname init_params	
			cap qui sfcross `lhs' `rhs', d(hn) `nocons' `cost' iter(50) difficult
			local _sfcross_conv "`e(converged)'"
			if _rc != 0 qui reg `lhs' `rhs', `nocons'	
			mat `init_params'=e(b)
			mat `init_beta' = `init_params'[1,1..`nsv_frontier']
...
if "`model'"=="pl81" {
		...
		tempname init_beta init_usigma init_vsigma temp
			...
			if "`svfrontier'"=="" {
				tempname init_params	
				cap qui sfcross `lhs' `rhs', d(hn) `nocons' `cost' iter(50)
				local _sfcross_conv "`e(converged)'"
				if _rc != 0 qui reg `lhs' `rhs', `nocons'	
				mat `init_params'=e(b)
				mat `init_beta' = `init_params'[1,1..`nsv_frontier']
...
if ("`model'" == "fe") {
	...
	if "`constraints'" == "" {	
		...
		qui xtreg `lhs' `rhs' [`weight' `__equal' `exp'] if `touse', fe vce(`vce'`clustervar')
		...
		qui cnsreg `lhs' `rhs' [`weight' `__equal' `exp'] if `touse', cons(`constraints')
